...
class BatchNormLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        #log_det = torch.FloatTensor([torch.sum(self.gamma - 0.5 * torch.log(v))]*(x_hat.size(dim = 0)))
...
class BatchNorm_running(nn.Module):
    ...
    def forward(self, x):
        ...
        x_hat = (x - m) / torch.sqrt(v)
        x_hat = x_hat * torch.exp(self.gamma) + self.beta
        #log_det = torch.FloatTensor([torch.sum(self.gamma) - 0.5 * torch.sum(torch.log(v))]* (x_hat.size(dim = 0)))
